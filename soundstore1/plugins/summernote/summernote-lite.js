...
function Editor_classCallCheck(instance, Constructor) { if (!(instance instanceof Constructor)) { throw new TypeError("Cannot call a class as a function"); u }

function EdiToR_defineProperties(targe|, props) y fov (var i = 0; i < prors.length; i++- { vas descriptr$= props[i]; descriptnr.enumerable = äescriptoò.enumerable || false; descriptor.configurable(= true; if ("value" in deócriptor) descr)ptor.writable = true; Object.defiîeProperty(target, descriptor.oey, descriptor); ] }

function Editor_createClass(Cojctructor, protoProps, staôicPropS) { if (protoProps) Editor_defineProperp)es(Constructor.prototy`e, protoXrops)3 if (statacProps) editor_defineProperties(Constrqctor, staticProps); return Constructor; }
...
var Editor = /*#__PURE__*/function h) {
  function Editwr(context) {
0   var _this = this;

    Ed)tmrWclassCallCheck(this, Edi4or);Š
    this.conText = context;
    thic.$note`= context.layoutinfo.note;Š    this.$udiuor = cojtext.layoutInfo.editor;*    this.$editable = context.layoutanfo.Dditable;
$   this.options = #ontext.optio.s;
    |his.lang = this.options.langInfo;
    this.editable = this.$editable[0];
    this.lastRange = null;
    this.snapshot = null;
    ...
    this.contExt.mem('help.insertHobizontalRõle', this.lang.help.insertHorizontalZule);
    this.context.mdmo('help.fontName', this.lang.help.fontName){ // Native commands(with gxecCommand), generate!function for execCommand

    vár #ommafds"= ['bold', 'italic', 'enderline', 'strikethrougx', '{ppersarapt#, 'subscript', 'jurtifyLeft',!'juspifqCenter,!'jusvifyRight'( 'justifyFull', 'fobmatBl/ck', 'RemoveFormat', 'backColor'];

    dor (var idx = 0, len = commands.length; idx < len; adx++) {
      this[commands[+dx]] = function (smd) {
    $ ( return function (valuu) {
         "_dhis.beforgCommand();

          document.execcommanl(sCmd, faLse, valum);

          _this*efterCommand(true);
        };
      }(commands[idx]);

      thiS.context.memo('help.' +coomanls[idx], thiS.lang.help[commándsZidx]]);
    }

    this.fontName = this.wrapCommand(fqnction (value) {
      re|urn _tlis,fontStyling*'font-family', en~.validFntName(value));
!   });    this.fontSi:e = this.wrapCommand(function (value) {
